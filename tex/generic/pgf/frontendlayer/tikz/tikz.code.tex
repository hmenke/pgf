...
% Replacement for semicolon at the end of the path. The parser will treat this
% token as if a semicolon was encoutered but if it is lost it doesn't typeset
% anything.
\pgfutil@protected\def\tikz@semicolon{\tikz@@semicolon}
\def\tikz@@semicolon{}
...
\def\tikz@@{%
  ...
  \fi%
  \tikz@next}%
\def\tikz@collectnormalsemicolon#1;{#1\tikz@semicolon\endtikzpicture\endgroup}
{
  \catcode`\;=\active
  \gdef\tikz@collectactivesemicolon#1;{#1\tikz@semicolon\endtikzpicture\endgroup}
}%
% End old code
...
\def\tikz@scoped@@{%
  ...
  \fi%
  \tikz@scoped@next}%
\def\tikz@scoped@collectnormalsemicolon#1;{#1\tikz@semicolon\endscope}
{
  \catcode`\;=\active
  \gdef\tikz@scoped@collectactivesemicolon#1;{#1\tikz@semicolon\endscope}
}%
...
\def\tikz@doopt{%
  ...
  \fi%
  \tikz@next}%
\long\def\tikz@eargnormalsemicolon<#1>#2;{\alt<#1>{\tikz@@command@path#2\tikz@semicolon}{\tikz@path@do@at@end}}%
{
  \catcode`\;=\active
  \long\global\def\tikz@eargactivesemicolon<#1>#2;{\alt<#1>{\tikz@@command@path#2\tikz@semicolon}{\tikz@path@do@at@end}}%
}
...
\def\tikz@handle{%
  \pgfutil@switch\pgfutil@ifx\pgf@let@token{%
    ...
    {\tikz@semicolon}{\let\pgfutil@next\tikz@finish}%
